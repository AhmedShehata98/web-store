...
const ApplicationPreview = ({ demoUrl, category, developer, name, rating, ratingCount, repoUrl, thumbnail }: Props) => {
  return (
    <div className='h-full min-h-full w-1/3 max-lg:w-full flex flex-col items-center justify-center dark:bg-dark-primary-200 dark:brightness-150 py-14 px-6 max-md:px-3 shadow-lg rounded-md'>
      ...
      <span className='flex mt-3 px-8 py-2 rounded-full dark:bg-dark-primary-300 text-lg shadow-xl font-medium capitalize dark:text-slate-300'>
        <p>{category}</p>
      </span>
    </div>
  );
