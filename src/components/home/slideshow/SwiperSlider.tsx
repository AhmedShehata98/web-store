...
const SwiperSlider = ({ data, isFetched, isLoading }: Props) => {
  return (
    <Swiper
      spaceBetween={18}
      slidesPerView={1.6}
          ...
          >
            ...
            <span className="absolute inset-0 flex items-end justify-start dark:bg-dark-primary-200 dark:bg-opacity-30">
              <div className="flex flex-col items-start justify-center mx-24 mb-24">
                <h3 className="max-w-[70%] text-4xl font-semibold mb-3 dark:text-white">
                  {slide.title}
                </h3>
                <p className="max-w-[70%] font-medium mb-3 dark:text-slate-300">
                  {slide.description}
                </p>
                <Link
                  href={slide.btnLink || "#"}
                ...
                >
                  ...
                  <small className="dark:text-white font-medium capitalize leading-3 me-3">
                    see more
                  </small>
                </Link>
              </div>
